...
from v1022_release import PORTABLE_ASSET, UPDATE_ASSET, VERSION
...
(ud / "update_status.json").write_text(json.dumps({
    ...
    "notes": "Nghia V10.0.22 TWO FLOOR SAFE-only top setup + shared behavior/recovery/jitter options; prior features preserved",
    "state": "up_to_date",
...
report = {
    "version": VERSION,
    "variant": "TwoFloorSafeUI",
    "base_release": "v10.0.21",
    "two_floor_top_ui_safe_only": True,
    "two_floor_left_right_derived_from_bottom_lane": True,
    "two_floor_simple_human_visible": True,
    "two_floor_fall_recovery_visible": True,
    "two_floor_loot_jitter_visible": True,
    "two_floor_transition_recovery_preserved": True,
...
(out / "preflight_report_v1022.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
(out / "preflight_hashes_v1022.txt").write_text(f"{psha}  {portable.name}\n{usha}  {updater.name}\n", encoding="utf-8")
print("V1022_PACKAGE_OK", psha, usha)

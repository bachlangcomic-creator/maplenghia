...
from v1023_release import PORTABLE_ASSET, UPDATE_ASSET, VERSION
...
(ud / "update_status.json").write_text(json.dumps({
    ...
    "notes": "Nghia V10.0.23 TWO FLOOR Natural Combat: V10.0.18-style run/attack cadence is preserved while travelling to floor transition points",
    "state": "up_to_date",
...
report = {
    "version": VERSION,
    "variant": "TwoFloorNatural",
    "base_release": "v10.0.22",
    "two_floor_transition_horizontal_uses_normal_combat_cadence": True,
    "two_floor_transition_direction_sync": True,
    "two_floor_vertical_recovery_safety_gate": True,
    "two_floor_wait_y_verification_preserved": True,
    "safe_ui_preserved": True,
    "pass_count_preserved": True,
...
(out / "preflight_report_v1023.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
(out / "preflight_hashes_v1023.txt").write_text(f"{psha}  {portable.name}\n{usha}  {updater.name}\n", encoding="utf-8")
print("V1023_PACKAGE_OK", psha, usha)

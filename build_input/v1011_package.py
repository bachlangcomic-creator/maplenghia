...
from v1011_release import PORTABLE_ASSET, UPDATE_ASSET, VERSION
    ...
    json.dumps(
        {
            ...
            "notes": "Nghia V10.0.11 Simple Loot Optional Coordinates",
            "state": "up_to_date",
        },
        indent=2,
        ensure_ascii=False,
...
if missing_required:
    raise SystemExit(
        "V10.0.11 updater payload missing launcher-required files: "
...
report = {
    "version": VERSION,
    "variant": "SimpleLootOptional",
    "base_release": "v10.0.10",
    "simple_loot_bottom_coords_optional": True,
    "none_loot_bottom_coords_optional": True,
    "pirate_bottom_coords_required": True,
    "legacy_pirate_route_still_requires_coords": True,
...
(out / "preflight_report_v1011.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
(out / "preflight_hashes_v1011.txt").write_text(
    f"{psha}  {portable.name}\n{usha}  {updater.name}\n", encoding="utf-8"
)
print("V1011_PACKAGE_OK", psha, usha)

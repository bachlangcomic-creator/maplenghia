...
if crash.exists():
    crash.unlink()
(root / 'version.json').write_text(json.dumps({'version': '10.0.2'}, indent=2) + '\n', encoding='utf-8')
(ud / 'update_status.json').write_text(json.dumps({
    'latest_version': '10.0.2',
    'local_version': '10.0.2',
    'notes': 'Nghia V10.0.2 Codex Canonical Stability',
    'state': 'up_to_date'
}, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')

out = Path('out')
out.mkdir(exist_ok=True)
portable = out / 'NghiaClient_V10.0.2_CodexCanonicalStability_Windows_Candidate.zip'
with ZipFile(portable, 'w', ZIP_DEFLATED, compresslevel=9) as z:
    for p in sorted(root.rglob('*')):
        if p.is_file():
            z.write(p, p.relative_to(root).as_posix())

updater = out / 'NghiaEdition_Update_v10.0.2.zip'
...
report = {
    'version': '10.0.2',
    'variant': 'CodexCanonicalStability',
    'worker_supervisor_generation_race_fixed': True,
    'codex_runtime_canonical': True,
...
print('V1002_PACKAGE_OK', psha, usha)

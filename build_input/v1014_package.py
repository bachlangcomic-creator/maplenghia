...
from v1014_release import PORTABLE_ASSET, UPDATE_ASSET, VERSION
    ...
    json.dumps(
        {
            ...
            "notes": "Nghia V10.0.14 Sell Trigger Arbitration",
            "state": "up_to_date",
        },
        indent=2,
        ensure_ascii=False,
...
if missing_required:
    raise SystemExit(
        "V10.0.14 updater payload missing launcher-required files: " + ", ".join(missing_required)
    )
...
report = {
    "version": VERSION,
    "variant": "SellTriggerArbitration",
    "base_release": "v10.0.13",
    "timer_mode_timer_only_trigger": True,
    "full_bag_mode_preserved_when_timer_off": True,
    "auto_sell_toggle_clears_stale_events": True,
    "auto_sell_toggle_restarts_timer_schedule": True,
...
(out / "preflight_report_v1014.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
(out / "preflight_hashes_v1014.txt").write_text(
    f"{psha}  {portable.name}\n{usha}  {updater.name}\n", encoding="utf-8"
)
print("V1014_PACKAGE_OK", psha, usha)

...
from v1017_release import PORTABLE_ASSET, UPDATE_ASSET, VERSION
    ...
    json.dumps(
        {
            ...
            "notes": "Nghia V10.0.17 Custom SAFE ENTRY + post-sale Return To Farm",
            "state": "up_to_date",
        },
        indent=2,
        ensure_ascii=False,
...
if missing_required:
    raise SystemExit("V10.0.17 updater payload missing required files: " + ", ".join(missing_required))
...
with ZipFile(updater, "w", ZIP_DEFLATED, compresslevel=9) as z:
    for p in sorted(payload_root.rglob("*")):
        if p.is_file():
            z.write(p, p.relative_to(root).as_posix())
    z.writestr("release.json", json.dumps(release_metadata, indent=2, ensure_ascii=False) + "\n")

psha = hashlib.sha256(portable.read_bytes()).hexdigest()
usha = hashlib.sha256(updater.read_bytes()).hexdigest()
report = {
    "version": VERSION,
    "variant": "SafeEntryReturn",
    "base_release": "v10.0.15",
    "safe_entry_custom_map": True,
    "post_sale_return_to_farm": True,
    "return_aligns_safe_entry_x_only_upstairs": True,
    "return_uses_down_plus_jump": True,
    "farm_lane_y_confirmation_before_resume": True,
    "wrong_floor_farm_return_included": False,
    "custom_maps_without_safe_entry_fallback": True,
    "built_in_maps_changed": False,
    "maps_json_changed": False,
    "v1015_human_rest_preserved": True,
    "v1014_sell_trigger_arbitration_preserved": True,
    "v1013_custom_map_sell_safe_preserved": True,
    "adaptive_y_preserved": True,
...
(out / "preflight_report_v1017.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
(out / "preflight_hashes_v1017.txt").write_text(
    f"{psha}  {portable.name}\n{usha}  {updater.name}\n", encoding="utf-8"
)
print("V1017_PACKAGE_OK", psha, usha)

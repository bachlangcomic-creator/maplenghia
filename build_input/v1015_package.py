...
from v1015_release import PORTABLE_ASSET, UPDATE_ASSET, VERSION
    ...
    json.dumps(
        {
            ...
            "notes": "Nghia V10.0.15 Human Rest 1000-1500ms",
            "state": "up_to_date",
        },
        indent=2,
        ensure_ascii=False,
...
if missing_required:
    raise SystemExit(
        "V10.0.15 updater payload missing launcher-required files: " + ", ".join(missing_required)
    )
...
report = {
    "version": VERSION,
    "variant": "HumanRestMs",
    "base_release": "v10.0.14",
    "human_rest_probability": 0.02,
    "human_rest_ms_min": 1000,
    "human_rest_ms_max": 1500,
    "human_jump_probability": 0.06,
    "human_jump_ms_min": 150,
    "human_jump_ms_max": 300,
    "human_continue_probability": 0.92,
...
(out / "preflight_report_v1015.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
(out / "preflight_hashes_v1015.txt").write_text(
    f"{psha}  {portable.name}\n{usha}  {updater.name}\n", encoding="utf-8"
)
print("V1015_PACKAGE_OK", psha, usha)

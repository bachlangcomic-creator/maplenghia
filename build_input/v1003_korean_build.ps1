...
if ($LASTEXITCODE -ne 0) { throw 'Portable Tesseract staging failed' }
...
try {
  ...
  python -m nuitka `
    ...
    --include-module=korean_question_helper `
...
if ($bytes.Length -lt 2 -or $bytes[0] -ne 0x4d -or $bytes[1] -ne 0x5a) { throw 'NghiaClientApp.exe is not PE/MZ' }
Write-Host 'V1003_KOREAN_PORTABLE_NUITKA_PE_OK'

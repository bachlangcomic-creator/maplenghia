...
from v1012_release import PORTABLE_ASSET, UPDATE_ASSET, VERSION
    ...
    json.dumps(
        {
            ...
            "notes": "Nghia V10.0.12 Watchdog Preview Type Fix",
            "state": "up_to_date",
        },
        indent=2,
        ensure_ascii=False,
...
if missing_required:
    raise SystemExit(
        "V10.0.12 updater payload missing launcher-required files: " + ", ".join(missing_required)
    )
...
report = {
    "version": VERSION,
    "variant": "WatchdogPreviewTypeFix",
    "base_release": "v10.0.11",
    "spotify_detection_result_not_cached_as_box": True,
    "preview_box_type_guard": True,
...
(out / "preflight_report_v1012.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
(out / "preflight_hashes_v1012.txt").write_text(
    f"{psha}  {portable.name}\n{usha}  {updater.name}\n", encoding="utf-8"
)
print("V1012_PACKAGE_OK", psha, usha)

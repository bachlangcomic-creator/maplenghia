...
exe=Path('portable/app_payload/NghiaClientApp.exe')
data=exe.read_bytes()
if not data.startswith(b'MZ'):
    raise SystemExit('not a PE/MZ executable')
for marker in (b'pyiboot01_bootstrap', b'_MEIPASS', b'PYZ-00.pyz'):
    if marker in data:
        raise SystemExit(f'PyInstaller marker present: {marker!r}')
print('V997_NUITKA_PE_OK', exe.stat().st_size)
'@ | python -
if ($LASTEXITCODE -ne 0) { throw 'PE verification failed' }
